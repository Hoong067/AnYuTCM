...
<head>
  ...
  <title>An Yu TCM – My Profile</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ── TOP NAV ── */
    .top-nav { position: fixed; top: 0; left: 50%; transform: translateX(-50%); width: 100%; max-width: 430px; height: var(--nav-h); background: rgba(245,240,232,0.95); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(123,63,26,0.1); display: flex; align-items: center; justify-content: space-between; padding: 0 20px; z-index: 100; }
    ...
    /* ── SCROLL CONTAINER ── */
    .scroll-body { position: fixed; top: var(--nav-h); bottom: var(--bottom-h); left: 50%; transform: translateX(-50%); width: 100%; max-width: 430px; overflow-y: auto; overflow-x: hidden; -webkit-overflow-scrolling: touch; scroll-behavior: smooth; }
    ...
    .profile-hero-deco { position: absolute; border-radius: 50%; border: 1px solid rgba(255,255,255,0.07); pointer-events: none; }
    ...
    .profile-stat-num { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 700; color: white; }
    .profile-stat-label { font-size: 11px; color: rgba(255,255,255,0.45); letter-spacing: 0.5px; margin-top: 2px; }
    ...
    .loyalty-card::before {
      ...
      right: -8px; bottom: -16px; line-height: 1;
    }

    .loyalty-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; }
    .loyalty-label { font-size: 10px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; color: rgba(255,255,255,0.65); }
    .loyalty-tier { font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; background: rgba(255,255,255,0.2); color: white; padding: 3px 10px; border-radius: 20px; }
    .loyalty-points { font-family: 'Playfair Display', serif; font-size: 30px; font-weight: 700; color: white; margin-bottom: 4px; }
    .loyalty-points span { font-size: 14px; font-weight: 400; color: rgba(255,255,255,0.7); margin-left: 4px; }
    .loyalty-bar-wrap { margin-top: 12px; }
    .loyalty-bar-label { display: flex; justify-content: space-between; font-size: 11px; color: rgba(255,255,255,0.65); margin-bottom: 6px; }
    .loyalty-bar { height: 6px; background: rgba(255,255,255,0.2); border-radius: 3px; overflow: hidden; }
    .loyalty-bar-fill { height: 100%; background: white; border-radius: 3px; width: 62%; }
    ...
    .menu-card {
      ...
      box-shadow: 0 2px 16px rgba(123,63,26,0.05);
    }
    ...
    .menu-item:not(:last-child) { border-bottom: 1px solid rgba(123,63,26,0.06); }
    .menu-item:active { background: rgba(123,63,26,0.03); }
    ...
    .menu-item-icon {
      ...
      display: flex; align-items: center; justify-content: center;
      font-family: 'Noto Serif SC', serif;
      font-size: 16px; color: var(--brown); flex-shrink: 0;
    }

    .menu-item-icon.gold-bg { background: rgba(184,138,68,0.1); color: var(--gold); }
    .menu-item-icon.green-bg { background: #e8f5ee; color: #2d7a47; }
    .menu-item-icon.red-bg { background: #fde8e8; color: #c0392b; }

    .menu-item-content { flex: 1; }
    .menu-item-title { font-size: 15px; font-weight: 600; color: var(--ink); margin-bottom: 1px; }
    .menu-item-desc { font-size: 12px; color: var(--brown-light); }

    .menu-item-arrow { color: rgba(123,63,26,0.25); }
    .menu-item-arrow svg { width: 14px; height: 14px; }
    ...
    .outlet-pref-label { font-size: 11px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; color: var(--brown-light); margin-bottom: 12px; }

    .outlet-pref-row { display: flex; gap: 10px; }
    ...
    .outlet-pref-btn.selected { background: var(--brown); border-color: var(--brown); }
    .outlet-pref-btn .op-name { font-family: 'Playfair Display', serif; font-size: 14px; font-weight: 600; color: var(--brown); }
    .outlet-pref-btn.selected .op-name { color: white; }
    .outlet-pref-btn .op-loc { font-size: 11px; color: var(--brown-light); margin-top: 2px; }
    .outlet-pref-btn.selected .op-loc { color: rgba(255,255,255,0.6); }

    /* ── SCROLL FOOTER ── */
    .scroll-footer { height: 24px; }

    /* ── BOTTOM NAV ── */
    .bottom-nav { position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 100%; max-width: 430px; height: var(--bottom-h); background: rgba(245,240,232,0.97); backdrop-filter: blur(20px); border-top: 1px solid rgba(123,63,26,0.1); display: flex; align-items: center; justify-content: space-around; padding: 0 8px; z-index: 100; }
...
<div class="scroll-body">
  ...
  <div class="profile-hero anim-1">
    <div class="profile-hero-deco ring-1"></div>
    <div class="profile-hero-deco ring-2"></div>
    <div class="profile-avatar-row">
      <div class="profile-avatar">安</div>
      <div class="profile-name-wrap">
        <div class="profile-name">Ahmad Rizal</div>
        <div class="profile-member">Silver Member</div>
      </div>
      <button class="profile-edit-btn">Edit</button>
    </div>
    <div class="profile-stats">
      <div class="profile-stat">
        <div class="profile-stat-num">5</div>
        <div class="profile-stat-label">Visits</div>
      </div>
      <div class="profile-stat">
        <div class="profile-stat-num">3</div>
        <div class="profile-stat-label">Services</div>
      </div>
      <div class="profile-stat">
        <div class="profile-stat-num">620</div>
  ...
  <!-- PREFERRED OUTLET -->
  <div class="section-label anim-3">Preferred Outlet</div>
  <div class="outlet-pref-card anim-3">
    <div class="outlet-pref-label">My Clinic</div>
    <div class="outlet-pref-row">
      <div class="outlet-pref-btn selected" onclick="setPref(this)">
        <div class="op-name">Kepong</div>
        <div class="op-loc">Jalan Kepong, KL</div>
      </div>
      <div class="outlet-pref-btn" onclick="setPref(this)">
        <div class="op-name">Bukit Jalil</div>
        <div class="op-loc">Bukit Jalil, KL</div>
      </div>
    </div>
  </div>

  <!-- ACCOUNT MENU -->
  <div class="section-label anim-3">Account</div>
  <div class="menu-card anim-3">
    <div class="menu-item">
      <div class="menu-item-icon">人</div>
      <div class="menu-item-content">
        <div class="menu-item-title">Personal Information</div>
        <div class="menu-item-desc">Name, phone, email & IC</div>
      </div>
      <div class="menu-item-arrow"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg></div>
    </div>
    <div class="menu-item">
      <div class="menu-item-icon gold-bg">歷</div>
      <div class="menu-item-content">
        <div class="menu-item-title">Health History</div>
        <div class="menu-item-desc">Allergies, conditions & notes</div>
      </div>
      <div class="menu-item-arrow"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg></div>
    </div>
    <div class="menu-item">
      <div class="menu-item-icon">約</div>
      <div class="menu-item-content">
        <div class="menu-item-title">My Bookings</div>
        <div class="menu-item-desc">Upcoming & past appointments</div>
      </div>
      <div class="menu-item-badge">2</div>
    </div>
  </div>

  <!-- SUPPORT MENU -->
  <div class="section-label anim-4">Support & Info</div>
  <div class="menu-card anim-4">
    <div class="menu-item">
      <div class="menu-item-icon green-bg">話</div>
      <div class="menu-item-content">
        <div class="menu-item-title">Contact Us</div>
        <div class="menu-item-desc">WhatsApp, call or email</div>
      </div>
      <div class="menu-item-arrow"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg></div>
    </div>
    <div class="menu-item">
      <div class="menu-item-icon">典</div>
      <div class="menu-item-content">
        <div class="menu-item-title">About An Yu TCM</div>
        <div class="menu-item-desc">Our story & commitment</div>
      </div>
      <div class="menu-item-arrow"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg></div>
    </div>
    <div class="menu-item">
      <div class="menu-item-icon">令</div>
      <div class="menu-item-content">
        <div class="menu-item-title">Privacy Policy</div>
        <div class="menu-item-desc">How we use your data</div>
      </div>
      <div class="menu-item-arrow"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg></div>
    </div>
    <div class="menu-item" style="cursor: pointer;" onclick="confirmLogout()">
      <div class="menu-item-icon red-bg">出</div>
      <div class="menu-item-content">
        <div class="menu-item-title" style="color: #c0392b;">Sign Out</div>
        <div class="menu-item-desc">Log out of your account</div>
      </div>
    </div>
  </div>
  ...
  <div style="text-align: center; padding: 24px 0 8px;">
    <div style="font-size: 11px; color: rgba(123,63,26,0.3); letter-spacing: 1px;">AN YU TCM · VERSION 1.0.0</div>
    <div style="font-family: 'Noto Serif SC', serif; font-size: 16px; color: rgba(123,63,26,0.15); margin-top: 4px; letter-spacing: 6px;">安鈺中医</div>
  </div>
...
<nav class="bottom-nav">
  ...
  <a class="tab-item active" href="{{ route('mobile.profile') }}">
    <svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
...
<div class="booking-sheet" id="bookingSheet">
  ...
  <div class="success-screen" id="successScreen">
    ...
    <button class="success-close" onclick="closeSheet()">Back to Profile</button>
  </div>
</div>
...
<script>
  ...
  function setPref(btn) {
    document.querySelectorAll('.outlet-pref-btn').forEach(b => b.classList.remove('selected'));
